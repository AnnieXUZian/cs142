...
app.get("/user/:id", function (request, response) {
  ...
  User.find({_id:id},"_id first_name last_name location description occupation",(err,user)=>{
    if(err){
...
app.get("/photosOfUser/:id", function (request, response) {
  const id = request.params.id;
  Photo.find({user_id:id},"_id user_id comments file_name date_time", async (err,photos)=>{
    if(err){
      return response.status(500).json(err);
    }if (photos.length === 0) {
    ...
    const result=await Promise.all (photos.map(async (photo)=>{
      const photo_comments =await Promise.all(photo.comments.map(async (comment) => {
